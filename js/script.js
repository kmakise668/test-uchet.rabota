let tablinks = document.querySelectorAll(".tab-link"),
    tabBtns = document.querySelector('.tab-btns'),
...
if (tablinks.length) {
    ...
    if (tabBtns) {
        ...
        nextBtn.addEventListener('click', function(e) {
            var current = document.querySelector('.tab-link.active'),
                tab_id = current.dataset.tablink;

            prevBtn.classList.remove('dn')

            if (tab_id != 'tab_' + tablinks.length + '') {
                current.classList.remove('active');
                document.querySelector("#" + tab_id).classList.remove("active");
                var id = current.nextElementSibling.dataset.tablink;
                document.querySelector('[data-tablink=' + id + ']').classList.add('active');
                document.querySelector("#" + id).classList.add("active");
            } else {
                this.setAttribute('disabled', 'disabled');
                setTimeout(() =>
                    window.location.href = "add-resume-success.html", 1500);
            }
        });
        ...
        prevBtn.addEventListener('click', function(e) {
            ...
            if (tab_id != 'tab_2') {
                current.classList.remove('active');
                document.querySelector("#" + tab_id).classList.remove("active");
                var id = current.previousElementSibling.dataset.tablink;
                document.querySelector('[data-tablink=' + id + ']').classList.add('active');
                document.querySelector("#" + id).classList.add("active");
            } else {
                current.classList.remove('active');
...
let skillItems = document.querySelectorAll('.select-dropdown.skill'),
    ...
    skill_0 = ["Бухгалтерский  учет" , "Налоговый учет", "Учет оплаты труда", "Учет ТМЗ", "Ведение первичной документации", "Ведение электронного документооборота", "Знание ЭСФ, СНТ, ОС", "Проверка контрагентов и сделок", "Отправка платежей по системе Клиент-Банк", "Бухгалтерские программы", "Авансовые отчеты"],
